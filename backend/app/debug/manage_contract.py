...
def create_contract(len_contract: int = 1):
    ...
    if len_contract > 2:
        typer.echo(f"contract len is greater than 2.")
        return
...
def create_user_brokers(len_contract: int = 1):
    ...
    cont_ids = list(cont_model.all(in_df=True).id.values)
    for i in range(4):
        if cont_break:
            cont_break -= 1
        else:
        ...
        params["api_secret"] = os.environ.get(f"DEBUG_BROKER_SECRET_KEY{i}", None)
        params["api_password"] = os.environ.get(f"DEBUG_BROKER_PASSPHRASE{i}", None)
...
def send_contract_to_station():
    ...
    contract_ids = cont_model.all(in_df=True).id.values
...
def delete_contracts():
    contracts = cont_model.all(all_=True, scalar=True)
    for cont in contracts:
        ...
        for pos in positions:
            ...
            AbortableTask(pos.task_id).abort()
...
def delete_gps():
    ...
    gps = gp_model.all(all_=True, scalar=True)
    for gp in gps:
        AbortableTask(gp.task_id).abort()
        typer.echo(f"Abort GP {gp.id} task.")

@app.command()
def init(len_contract: int = 1) -> None:
